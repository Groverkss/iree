...
struct CuteDistributeMultiReduction final
    : OpDistributionPattern<vector::MultiDimReductionOp> {
  CuteDistributeMultiReduction(MLIRContext *ctx, Value threadId,
  ...
  LogicalResult matchAndRewrite(vector::MultiDimReductionOp reduceOp,
    ...
    if (!layout) {
      return rewriter.notifyMatchFailure(reduceOp, "not a CuTe layout");
    }
    ...
    //
    // Example: layout with reduction dims having thread leaves
    //   (4):(2) and (4):(16) -> sort desc -> (4,4):(16,2) -> coalesce ->
    //   (8):(2) (merged because 4*2 = 8 and next stride 16 = 8*2).
    //   But (4):(2) and (2):(64) -> (2,4):(64,2) -> no merge (4*2!=64).
    // =================================================================
    ...
    //
    // The tiler size K = number of contiguous flat positions that fit
    // within one subgroup. This works because:
    //  - The grid is in lex order (smallest stride rightmost = fastest).
    //  - Flat positions 0..K-1 sweep the smallest strides first, keeping
    //    thread IDs within sgSize.
    //  - logicalDivide(grid, (K):(1)) = (withinSG, acrossSG) where
    //    mode 0 maps to thread IDs all within one subgroup, and mode 1
    //    maps to different subgroups.
    // =================================================================
    ...
    //
    // Coalesce the within-SG layout to get the minimal set of clusters.
    // Each mode becomes one gpu.subgroup_reduce call with that mode's
    // (size, stride) as cluster parameters. Multiple modes arise when
    // parallel dims create gaps in the reduction thread grid.
    // =================================================================
    struct ClusterInfo {
      int64_t size;
      int64_t stride;
    ...
    //
    // After the within-SG shuffle, each thread holds a partial sum, but
    // partial sums are distributed across subgroups according to
    // acrossSGLayout. We write to shared memory with the SG stride,
    // read back with stride 0 (broadcast), then locally reduce.
    //
    // Write layout: (parallel_modes..., sg_shape) : (..., sg_stride)
    // Read layout:  (parallel_modes..., sg_shape) : (..., 0)
    // =================================================================
    ...
    // The SG mode is all-thread (stride != 0), so it contributes a
    // size-1 dim to the distributed shape -- just an unsqueeze.
    ...
    // CuteDistributeToLayout handles the shared memory
    ...
    // The SG dims have stride 0 in readLayout (broadcast), so the
    // framework will distribute this as a purely local reduce.
...
void populateCuteDistributeReductionPatterns(RewritePatternSet &patterns,
                                             Value threadId,
                                             int64_t subgroupSize) {
  patterns.add<CuteDistributeMultiReduction>(patterns.getContext(), threadId,
                                             subgroupSize);
}
