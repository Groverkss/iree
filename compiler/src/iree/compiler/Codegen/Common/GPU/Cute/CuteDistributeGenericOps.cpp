...
struct CuteDistributeShapeCast final
  ...
  LogicalResult matchAndRewrite(vector::ShapeCastOp shapeCast,
    ...
    if (!srcLayout || !dstLayout) {
      return rewriter.notifyMatchFailure(shapeCast, "not a CuTe layout");
    }
...
struct CuteDistributeBroadcast final
  ...
  LogicalResult matchAndRewrite(vector::BroadcastOp broadcastOp,
    ...
    auto dstLayout = dyn_cast<PackLayoutAttr>(signature[dst]);
    if (!dstLayout) {
      return rewriter.notifyMatchFailure(broadcastOp, "not a CuTe layout");
    }
...
struct CuteDistributeTranspose final
  ...
  LogicalResult matchAndRewrite(vector::TransposeOp transposeOp,
    ...
    auto srcLayout = dyn_cast<PackLayoutAttr>(signature[src]);
    if (!srcLayout) {
      return rewriter.notifyMatchFailure(transposeOp, "not a CuTe layout");
    }
...
struct CuteDistributeStep final : OpDistributionPattern<vector::StepOp> {
  CuteDistributeStep(MLIRContext *ctx, Value threadId)
  ...
  LogicalResult matchAndRewrite(vector::StepOp stepOp,
    ...
    if (!layout) {
      return rewriter.notifyMatchFailure(stepOp, "not a CuTe layout");
    }
...
void populateCuteDistributeGenericPatterns(RewritePatternSet &patterns,
                                           Value threadId) {
  patterns.add<CuteDistributeShapeCast, CuteDistributeBroadcast,
               CuteDistributeTranspose>(patterns.getContext());
  patterns.add<CuteDistributeStep>(patterns.getContext(), threadId);
}
