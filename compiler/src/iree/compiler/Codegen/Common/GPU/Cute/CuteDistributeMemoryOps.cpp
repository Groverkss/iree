...
//===----------------------------------------------------------------------===//
// CuteDistributeTransferRead
//===----------------------------------------------------------------------===//

struct CuteDistributeTransferRead final
    : OpDistributionPattern<vector::TransferReadOp> {
  CuteDistributeTransferRead(MLIRContext *ctx, Value threadId)
  ...
  LogicalResult matchAndRewrite(vector::TransferReadOp readOp,
    ...
    if (!layout) {
      return rewriter.notifyMatchFailure(readOp, "not a CuTe layout");
    }
...
//===----------------------------------------------------------------------===//
// CuteDistributeTransferWrite
//===----------------------------------------------------------------------===//

struct CuteDistributeTransferWrite final
    : OpDistributionPattern<vector::TransferWriteOp> {
  CuteDistributeTransferWrite(MLIRContext *ctx, Value threadId)
      : OpDistributionPattern(ctx), threadId(threadId) {}
  ...
  LogicalResult matchAndRewrite(vector::TransferWriteOp writeOp,
    ...
    if (!layout) {
      return rewriter.notifyMatchFailure(writeOp, "not a CuTe layout");
    }
...
//===----------------------------------------------------------------------===//
// CuteDistributeToLayout (shared-memory fallback)
//===----------------------------------------------------------------------===//

struct CuteDistributeToLayout final
    : OpDistributionPattern<IREE::VectorExt::ToLayoutOp> {
  CuteDistributeToLayout(MLIRContext *ctx, Value threadId)
      : OpDistributionPattern(ctx), threadId(threadId) {}
...
void populateCuteDistributeMemoryOpPatterns(RewritePatternSet &patterns,
                                            Value threadId) {
  patterns.add<CuteDistributeTransferRead, CuteDistributeTransferWrite,
               CuteDistributeToLayout>(patterns.getContext(), threadId);
}
