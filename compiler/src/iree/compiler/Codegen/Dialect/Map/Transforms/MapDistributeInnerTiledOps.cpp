...
struct MapDistributeInnerTiled final
  ...
  LogicalResult matchAndRewrite(IREE::Codegen::InnerTiledOp tiledOp,
    ...
    for (Value input : tiledOp.getInputs()) {
      ...
      if (!layout) {
        return rewriter.notifyMatchFailure(tiledOp, "missing PackLayout");
      }
...
void populateMapDistributeInnerTiledPatterns(RewritePatternSet &patterns) {
  patterns.add<MapDistributeInnerTiled>(patterns.getContext());
}
