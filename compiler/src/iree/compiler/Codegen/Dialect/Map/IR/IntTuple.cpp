...
bool isCongruent(Attribute a, Attribute b) {
  ...
  for (auto [ea, eb] : llvm::zip(arrA, arrB)) {
...
int32_t innerProduct(Attribute coord, Attribute stride) {
  ...
  int32_t sum = 0;
  for (auto [c, s] : llvm::zip(coordArr, strideArr)) {
...
// Port of _pycute/int_tuple.py:149 (suffix_product)
...
// Port of _pycute/int_tuple.py:177 (idx2crd)
...
int32_t crd2idx(ArrayRef<int32_t> coord, Attribute shape, Attribute stride) {
  ...
  int32_t result = 0;
  for (auto [c, s] : llvm::zip(coord, strides)) {
    result += c * s;
  }
  return result;
}
