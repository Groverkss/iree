...
TEST_F(PackMapTest, ComplementWithGap) {
  // (4) : (2) with cotarget=8 -> gap of 2 at stride 1
  auto layout = make({4}, {2});
  auto comp = layout.complement(8);
  // complement should fill the gap
  EXPECT_GE(comp.getSize(), 1);
}
...
TEST_F(PackMapTest, LogicalDivideBasic) {
  // Divide (32) : (1) by tiler (4) : (1) -> rank-2 result
...
//===----------------------------------------------------------------------===//
// ZippedDivide / TiledDivide
//===----------------------------------------------------------------------===//

TEST_F(PackMapTest, ZippedDivide) {
  auto layout = make({32}, {1});
  auto tiler = make({4}, {1});
  auto result = layout.zippedDivide(tiler);
  EXPECT_EQ(result.getRank(), 2);
  EXPECT_EQ(result.getSize(), 32);
}
...
TEST_F(PackMapTest, TiledDivide) {
  ...
  EXPECT_GE(result.getRank(), 2);
  EXPECT_EQ(result.getSize(), 32);
}

//===----------------------------------------------------------------------===//
// ZippedProduct / TiledProduct
//===----------------------------------------------------------------------===//

TEST_F(PackMapTest, ZippedProduct) {
  auto layout = make({4}, {1});
  auto tiler = make({8}, {1});
  auto result = layout.zippedProduct(tiler);
  EXPECT_EQ(result.getRank(), 2);
  EXPECT_EQ(result.getSize(), 32);
}
