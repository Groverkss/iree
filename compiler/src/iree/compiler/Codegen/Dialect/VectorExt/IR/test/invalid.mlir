...
func.func @reduce_inputs_different_shapes(%a: vector<4x64xf32>,
  ...
  return %result#0, %result#1 : vector<4xf32>, vector<8xi32>
}
...
func.func @reduce_yield_count_mismatch(%input: vector<4x64xf32>,
  ...
  %0 = iree_vector_ext.associative_reduce
    ...
    ^bb0(%lhs: f32, %rhs: f32):
      ...
      iree_vector_ext.yield %r, %r : f32, f32
  } -> vector<4xf32>
  return %0 : vector<4xf32>
}
...
func.func @reduce_yield_type_mismatch(%in_val: vector<4x64xf32>,
  ...
  %result:2 = iree_vector_ext.associative_reduce
    ...
    ^bb0(%lv: f32, %li: i32, %rv: f32, %ri: i32):
      ...
      // expected-error @+1 {{expected yield operand #1 to have type 'i32', but got 'f32'}}
      iree_vector_ext.yield %ov, %ov : f32, f32
  } -> vector<4xf32>, vector<4xi32>
  return %result#0, %result#1 : vector<4xf32>, vector<4xi32>
}
...
func.func @scan_dim_out_of_range(%input: vector<4x64xf32>) -> vector<4x64xf32> {
  // expected-error @+1 {{reduction dimension 2 is out of range [0, 2)}}
...
func.func @scan_wrong_combiner_args(%input: vector<4x64xf32>) -> vector<4x64xf32> {
  // expected-error @+1 {{combiner region must have exactly 2 arguments}}
  %0 = iree_vector_ext.associative_scan
...
func.func @scan_inputs_different_shapes(%a: vector<4x64xf32>,
                                        %b: vector<8x64xi32>)
    -> (vector<4x64xf32>, vector<8x64xi32>) {
  // expected-error @+1 {{all inputs must have the same shape}}
  ...
  return %result#0, %result#1 : vector<4x64xf32>, vector<8x64xi32>
}
