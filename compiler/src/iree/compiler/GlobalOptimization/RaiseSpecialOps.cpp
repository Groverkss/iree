...
struct RaiseGenericOpToView final : OpRewritePattern<linalg::GenericOp> {
  using OpRewritePattern::OpRewritePattern;

  LogicalResult matchAndRewrite(linalg::GenericOp genericOp,
                                PatternRewriter &rewriter) const override {
    FailureOr<linalg::GenericOp> newOp = tryRaiseToView(genericOp, rewriter);
    if (failed(newOp)) {
      return failure();
    }
...
struct RaiseSpecialOpsPass
  ...
  void runOnOperation() override {
    ...
    // First walk the IR and try to raise any slice-like generics to tensor.
    {
      ...
      IREE::LinalgExt::populateTensorExtractToInputRaisingPatterns(patterns);
      patterns.add<RaiseGenericOpToView>(context);
